...
export default function PostsListClient({ 
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
      <div className="surface rounded-2xl overflow-hidden min-w-0">
        <div className="px-4 py-6 sm:px-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between overflow-hidden">
          <div>
          ...
          <Link
            ...
            className="flex w-full max-w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-indigo-500 via-sky-500 to-fuchsia-500 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-indigo-500/30 hover:opacity-90 transition whitespace-normal break-words sm:w-auto"
          >
            <Plus className="h-4 w-4 flex-shrink-0" />
            <span className="min-w-0">New Post</span>
          </Link>
        </div>
                      ...
                      <div className="flex items-center justify-between gap-3 sm:hidden">
                        ...
                        <div className="flex items-center gap-4">
                          <Link
                            ...
                            className="inline-flex items-center text-sm font-medium text-sky-300 hover:text-sky-200 whitespace-nowrap"
                          >
                          ...
                          <button
                            ...
                            className="inline-flex items-center text-sm font-medium text-rose-300 hover:text-rose-200 whitespace-nowrap"
                          >
              ...
              {seoNeedsAttention.slice(0, 5).map((post) => (
                <li key={post.id} className="flex flex-col gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-3">
                  <div className="flex min-w-0 items-center justify-between gap-3">
                    <p className="min-w-0 flex-1 text-white font-semibold break-words sm:truncate">{post.title}</p>
                    <Link
                      href={`/admin/posts/${post.id}/edit`}
